...
def sort_patient(
    ...
    if sort_by not in valid_fields:
        raise HTTPException(
            status_code=400, detail=f"Invalid fields select from {valid_fields} "
        )

    if order not in ["asc", "desc"]:
        raise HTTPException(
            status_code=400, detail=f"Invalid order select between asc and desc"
        )
...
@app.post("/create")
def create_patint(patient: Patient):
    ...
    if patient.id in data:
        raise HTTPException(status_code=400, detail="Pateint already exixts")
    ...
    return JSONResponse(
        status_code=201, content={"message": "patient created succesfully"}
    )
